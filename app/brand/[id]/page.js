      ...
      try {
        ...
        if (response.success && response.data) {
          const brand = response.data.find(
            (c) => String(c.id) === String(brandId)
          );
          if (brand) {
            setBrandName(brand.name);
            setBannerImage(brand.banner_image || brand.banner || null);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        // Fetch products based on brand route context
        let response = await getBrandwiseProducts(brandId, currentPage, 40);
